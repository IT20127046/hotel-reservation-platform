...
export default class viewBookedDetails extends Component {
  constructor(props) {
    super(props);
  ...
  retrieveReservedRooms() {
    ...
    axios.get(`http://localhost:5000/reservedroom/roomno/${id}`).then((res) => {
      if (res.data.success) {
        this.setState({
          roomreserveddetails: res.data.exsitingRooms,
        });
